...
const resolve = () => {
    ...
    const bindingsFound = platformFound.bindings.find(isArchMatch);

    if (!bindingsFound) {
        throw new Error(`Unsupported architecture on ${platformFound.platform}: ${process.arch}`);
    }

    const hasLocalFile = existsSync(path.join(__dirname, `swc.${bindingsFound.key}.node`));
    return hasLocalFile ? require(`./swc.${bindingsFound.key}.node`) : require(`@next/swc-${bindingsFound.key}`);
};
